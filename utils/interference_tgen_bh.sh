...
if [ ! -d "./interference/mal_traf/bandwidth_hog/" ]
  then
    mkdir ./interference
    mkdir ./interference/mal_traf
    mkdir ./interference/mal_traf/bandwidth_hog
fi
...
sshpass -p "scope" scp traffic_gen_bh.py $1:/root/traffic_gen/
...
do
  ...
  sshpass -p "scope" ssh -tt $1 "cd /root/traffic_gen && python traffic_gen_bh.py --eNB -f ${t}" &   # this will have to let the next command go through
  gNB_PID=$!
  echo "Sleep for 5 secs"
  sleep 5  # let's wait few seconds
  echo "***** Run traffic on UE *****"
  sshpass -p "scope" ssh -tt $2 "cd /root/traffic_gen && python traffic_gen_bh.py -f ${t}" &
  UE_PID=$!
  ...
  sshpass -p "scope" scp $1:/root/radio_code/scope_config/metrics/csv/101*_metrics.csv ./interference/mal_traf/bandwidth_hog/${tracename}
